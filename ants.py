...
class Insect:
    ...
    def __init__(self, health, place=None, slowness=0, scareness=0, scared_before=False):
        """Create an Insect with a health amount and a starting PLACE."""
        self.health = health
        self.place = place  # set by Place.add_insect and Place.remove_insect
        self.slowness = slowness
        self.scareness = scareness
        self.scared_before = scared_before
...
class Ant(Insect):
    ...
    def __init__(self, health=1, buff=False):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
        elif (place.ant.is_container and place.ant.ant_contained is not None) or (not place.ant.is_container and not self.is_container) or (place.ant.is_container and self.is_container):
            assert place.ant is None, 'Two ants in {0}'.format(place)
        elif (place.ant.is_container and not self.is_container) or (self.is_container and not place.ant.is_container):
            # BEGIN Problem 8
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if random_bee(self.place.bees) != None and self.min_range <= 0:
            return random_bee(self.place.bees)
        count = 1
        while count <= self.max_range:
            if next_place.is_hive:
                break
            elif random_bee(next_place.bees) != None and self.min_range <= count:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for index in range(len(copy)):
            self.place.bees[index-(len(copy)-len(self.place.bees))].reduce_health(amount)
        copy = list(self.place.bees)
        super().reduce_health(amount)
        if self.health <= 0:
            for index in range(len(copy)):
                place_copy.bees[index-(len(copy)-len(place_copy.bees))].reduce_health(self.damage)
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        copy = list(self.place.bees)
        for index in range(len(copy)):
            self.place.bees[index-(len(copy)-len(self.place.bees))].reduce_health(self.damage)
        if self.ant_contained is not None:
            self.ant_contained.action(gamestate)
# END Problem 9
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_ant == 1:
            return None
        gamestate.queen_ant += 1
    ...
    def action(self, gamestate):
        ...
        if self.place.ant.name == 'Tank' and self.place.ant.buff is False:
        ...
        while True:
            ...
            if next_place.ant is not None:
                if next_place.ant.is_container:
                    if next_place.ant.ant_contained is not None and next_place.ant.ant_contained.buffed is False:
                        next_place.ant.ant_contained.damage *= 2
                        next_place.ant.ant_contained.buffed = True
                if next_place.ant.buffed is False:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slowness>0 and gamestate.time%2 != 0:
            pass
        elif self.blocked():
            self.sting(self.place.ant)
        elif self.scareness>0 and not self.scared_before:
            self.scareness -= 1
            if self.scareness == 0:
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2, queen_ant=0):
